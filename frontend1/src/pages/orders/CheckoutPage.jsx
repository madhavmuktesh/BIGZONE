...
const CheckoutPage = () => {
    ...
    ? [
        {
          product: {
            ...
            images: [{ url: buyNowItem.image }],
          },
          quantity: buyNowItem.quantity,
          priceAtAddition: buyNowItem.productprice,
        },
      ]
    : cart?.items || [];

  const subtotal = isBuyNow
    ? (buyNowItem?.productprice || 0) * (buyNowItem?.quantity || 0)
    : cart?.totalPrice || 0;
      ...
      if (data.success) {
        ...
        if (addresses.length > 0) {
          const defaultAddress =
            addresses.find((a) => a.makeDefault) || addresses[0];
  ...
  useEffect(() => {
    ...
    fetchAddresses();
  }, [user, token, cart, navigate, isBuyNow]);
      ...
      if (isBuyNow) {
        const orderPayload = {
          ...
          buyNowItem: {
            ...
            quantity: buyNowItem.quantity,
            priceAtPurchase: buyNowItem.productprice,
          },
        };

        response = await fetch(`${API_BASE_URL}/orders/buy-now`, {
          method: "POST",
          credentials: "include",
          headers: {
        ...
        response = await fetch(`${API_BASE_URL}/orders/create`, {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="cart-layout">
          <div className="cart-items-list checkout-form-card">
            ...
            <form onSubmit={handlePlaceOrder} className="checkout-form">
              ...
              <div className="checkout-note">Payment method: Cash on Delivery.</div>
          ...
          <aside className="order-summary-card">
            ...
            <div className="summary-rows">
              {checkoutItems.map((item, idx) => (
                <div className="summary-row" key={idx}>
                  ...
                  <span className="value">
                    ₹
                    {(
                      (item.priceAtAddition || item.product?.productprice || 0) *
                      item.quantity
                    ).toFixed(2)}
                  </span>
                </div>
              ...
              <div className="summary-row">
                <span className="label">Items ({checkoutItems.length})</span>
                <span className="value">₹{subtotal.toFixed(2)}</span>
              </div>
            </div>

            <EcoCartSummary items={checkoutItems} />

            <div className="summary-total summary-row">
              <span className="label">Total to Pay</span>
              <span className="value">₹{subtotal.toFixed(2)} (COD)</span>
            </div>
